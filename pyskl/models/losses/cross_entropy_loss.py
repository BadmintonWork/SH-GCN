    ...
    def _forward(self, cls_score, label, **kwargs):
        ...
        if cls_score.size() == label.size():
            ...
            if self.label_smoothing > 0.0:
                assert cls_score.dim() == 2, 'Only support 2-dim logits for smoothing'
                N, K = cls_score.size(0), cls_score.size(1)
                eps = self.label_smoothing
                with torch.no_grad():
                    smooth = torch.full(
                        (N, K), eps / max(K - 1, 1),
                        device=cls_score.device, dtype=cls_score.dtype)
                    smooth.scatter_(1, label.view(-1, 1), 1.0 - eps)
                # 递归调用 soft 分支（上面的 if 分支）
                return self._forward(cls_score, smooth, **kwargs)
